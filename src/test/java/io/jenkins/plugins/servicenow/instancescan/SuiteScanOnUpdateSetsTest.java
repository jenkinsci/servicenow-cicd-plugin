...
public class SuiteScanOnUpdateSetsTest extends TestCase {

    @Mock
    private ServiceNowAPIClient apiClient;

    private SuiteScanOnUpdateSets testedSuiteScan = new SuiteScanOnUpdateSets();

    @Test
    public void shouldBeApplicableForSuiteScan() {
        // given
        ScanType scanType = ScanType.scanWithSuiteOnUpdateSets;
    ...
    public void shouldNotBeApplicableForOtherScanTypes() {
        ...
        for(ScanType scanType : ScanType.values()) {
            if(scanType == ScanType.scanWithSuiteOnUpdateSets) {
                continue;
            }
            // when
    ...
    public void shouldSendRequest() throws IOException, URISyntaxException {
        ...
        given(apiClient.executeScanWithSuiteOnUpdateSet(eq(param1), eq(param2))).willReturn(expectedResult);
    ...
    public void shouldSendRequestWithEmptyParameters() throws IOException, URISyntaxException {
        ...
        Result expectedResult = new Result();
        given(apiClient.executeScanWithSuiteOnUpdateSet(isNull(), isNull())).willReturn(expectedResult);
